...
<script lang="ts" setup>
  ...
  import { getSummmaryList, toSummmary, myApplypr } from '@/api/project/list';
  import { columns } from './columnsAll';
  ...
  const loadDataTable = async (res) => {
    ...
    if (!formParams.pass_or_return || formParams.pass_or_return == 0) {
      return await getSummmaryList({ ...formParams, ...params.value, ...res });
    }
    //  else {
